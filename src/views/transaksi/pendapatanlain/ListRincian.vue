    ...
    <u-row flex1 class="w-full" cols="4">
      <u-row flex1 class="w-full">
        <u-col gap="gap-1">
          <u-row gap="gap-2">
            <!-- <span>💊 </span> -->
            <u-text size="md" class="font-bold">{{ item?.keterangan || '-' }}</u-text>
          </u-row>
          <u-row gap="gap-1">
            <u-badge variant="warning">Rp. {{ formatRupiah(item?.nilai) }}</u-badge>
          </u-row>
        </u-col>
      </u-row>
      <u-row gap="gap-0">
...
const handleDelete = async (item) => {
  const payload = {
    notransaksi: props.store.form?.notrans,
    id: item?.id ?? null,
  }


  try {
    const resp = await api.post(`api/v1/transactions/pendapatanlain/hapus`, payload)
    console.log('respon hapus', resp)
