      ...
      <u-view padding="px-3 py-3" flex1 class="w-full cursor-pointer" @click="handleEdit(item)">
        <u-row flex1 class="w-full">
            ...
            <u-row flex1>
              <u-col gap="gap-0" class="w-full">
                <u-row flex1>
                  ...
                  <u-text class="font-bold" color="text-gray-500">{{ item?.notrans }}</u-text>
                </u-row>
                <u-row>
                  <u-text class="" color="text-gray-500">Penerimaan dari {{ item?.dari }}</u-text>
                </u-row>
          ...
          <u-col align="items-end" gap="gap-0" class="" padding="p-0">
            <u-icon :name="item?.kunci ? 'lock' : 'lock-open'" size="18" class="mb-1" :class="!item?.kunci ? 'text-success' : 'text-danger'" />
