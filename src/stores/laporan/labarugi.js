...
export const useLaporanLabarugiStore = defineStore('laporan-labarugi-store', {
  state: () => ({
    items: null,
    params: {
      q: '',
  ...
  actions: {
    ...
    async fetchData(extraParams = {}) {
      ...
      try {
        ...
        const res = await api.get('/api/v1/laporan/laba-rugi/get-report', { params })
        console.log(`resp getList : `, res);
        this.items = res.data?.data ?? res.data ?? null
...
if (import.meta.hot) {
  import.meta.hot.accept(acceptHMRUpdate(useLaporanLabarugiStore, import.meta.hot))
